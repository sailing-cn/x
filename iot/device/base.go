...
func (device *baseIotDevice) Init() bool {
	...
	options.SetPassword(encrypt.HmacSha256(device.Password, timestamp.TimestampString()[:10]))
	...
	commandHandler := func(client mqtt.Client, message mqtt.Message) {
		go func() {
			...
			if handleFlag {
				...
				res = convert.ToString(iot.CommandResponse{
					ResultCode: 0,
			...
			if token := device.Client.Publish(mqtt2.FormatTopic(iot.CommandResponseTopic, device.Id)+mqtt2.GetTopicRequestId(message.Topic()), 1, false, res); token.Wait() && token.Error() != nil {
	...
	propertiesSetHandler := func(client mqtt.Client, message mqtt.Message) {
		go func() {
			...
			if handleFlag {
				...
				res = convert.ToString(response)
			} else {
				response.ResultCode = 1
				response.ResultDesc = "Set properties failed."
				res = convert.ToString(response)
			}
			if token := device.Client.Publish(mqtt2.FormatTopic(iot.PropertiesSetResponseTopic, device.Id)+mqtt2.GetTopicRequestId(message.Topic()), device.qos, false, res); token.Wait() && token.Error() != nil {
				glog.Warningf("unmarshal platform properties set request failed,device id = %s，message = %s", device.Id, message)
			}
		}()
	}
...
func (device *baseIotDevice) createPropertiesQueryMqttHandler() func(client mqtt.Client, message mqtt.Message) {
	propertiesQueryHandler := func(client mqtt.Client, message mqtt.Message) {
		go func() {
			...
			responseToPlatform := convert.ToString(queryResult)
			if token := device.Client.Publish(mqtt2.FormatTopic(iot.PropertiesQueryResponseTopic, device.Id)+mqtt2.GetTopicRequestId(message.Topic()), device.qos, false, responseToPlatform); token.Wait() && token.Error() != nil {
...
func (device *baseIotDevice) createPropertiesQueryResponseMqttHandler() func(client mqtt.Client, message mqtt.Message) {
	propertiesQueryResponseHandler := func(client mqtt.Client, message mqtt.Message) {
		...
		if json.Unmarshal(message.Payload(), propertiesQueryResponse) != nil {
			glog.Warningf("device %s unmarshal property response failed,message %s", device.Id, convert.ToString(message))
		}
...
func (device *baseIotDevice) subscribeDefaultTopics() {
	// 订阅平台命令下发topic
	topic := mqtt2.FormatTopic(iot.CommandDownTopic, device.Id)
	...
	// 订阅平台消息下发的topic
	topic = mqtt2.FormatTopic(iot.MessageDownTopic, device.Id)
	...
	// 订阅平台设置设备属性的topic
	topic = mqtt2.FormatTopic(iot.PropertiesSetRequestTopic, device.Id)
	...
	// 订阅平台查询设备属性的topic
	topic = mqtt2.FormatTopic(iot.PropertiesQueryRequestTopic, device.Id)
	...
	// 订阅查询设备影子响应的topic
	topic = mqtt2.FormatTopic(iot.DeviceShadowQueryResponseTopic, device.Id)
	...
	// 订阅平台下发到设备的事件
	topic = mqtt2.FormatTopic(iot.PlatformEventToDeviceTopic, device.Id)
	...
	handler := func(client mqtt.Client, message mqtt.Message) {
		...
		for _, entry := range data.Services {
			...
			case "add_sub_device_notify":
				...
				if json.Unmarshal([]byte(convert.ToString(entry.Paras)), subDeviceInfo) != nil {
					continue
				}
			...
			case "get_upload_url_response":
				...
				if json.Unmarshal([]byte(convert.ToString(entry.Paras)), fileResponse) != nil {
					continue
				}
				device.fileUrls[fileResponse.ObjectName+iot.FileActionUpload] = fileResponse.Url
			case "get_download_url_response":
				fileResponse := &iot.FileResponseServiceEventParas{}
				if json.Unmarshal([]byte(convert.ToString(entry.Paras)), fileResponse) != nil {
					continue
				}
				device.fileUrls[fileResponse.ObjectName+iot.FileActionDownload] = fileResponse.Url
			case iot.VERSION_QUERY:
				// 查询软固件版本
				log.Infof("查询设备版本:%s", time.Now().String())
				device.reportVersion()

			case iot.FIRMWARE_UPGRADE:
				upgradeInfo := &iot.UpgradePara{}
				if json.Unmarshal([]byte(convert.ToString(entry.Paras)), upgradeInfo) != nil {
					continue
				}
				device.upgradeDevice(iot.FIRMWARE_UPGRADE, upgradeInfo)

			case iot.SOFTWARE_UPGRADE:
				upgradeInfo := &iot.UpgradePara{}
				if json.Unmarshal([]byte(convert.ToString(entry.Paras)), upgradeInfo) != nil {
					continue
				}
				device.upgradeDevice(iot.SOFTWARE_UPGRADE, upgradeInfo)
			case iot.CONFIG_UPGRADE:
				upgradeInfo := &iot.UpgradePara{}
				err := json.Unmarshal([]byte(convert.ToString(entry.Paras)), upgradeInfo)
				if err != nil {
					log.Errorf("解析平台事件参数失败:%s", err.Error())
					continue
				}
				device.upgradeDevice(iot.CONFIG_UPGRADE, upgradeInfo)
			case iot.MESSAGE_RECEUVED:
				messageResponse := &iot.Message{}
				err := json.Unmarshal([]byte(convert.ToString(entry.Paras)), messageResponse)
				if err != nil {
			...
			case "log_config":
				...
				//if json.Unmarshal([]byte(convert.ToString(entry.Paras)), logConfig) != nil {
				//	continue
				//}
				//
				//lcc := &LogCollectionConfig{
...
func (device *baseIotDevice) reportVersion() {
	...
	device.Client.Publish(mqtt2.FormatTopic(iot.DeviceToPlatformTopic, device.Id), device.qos, false, convert.ToString(data))
}
...
func (device *baseIotDevice) upgradeDevice(upgradeType iot.EventType, upgradeInfo *iot.UpgradePara) {
	...
	dataEntry := iot.ServiceEvent{
		ServiceId: iot.OTA,
		EventType: iot.UPGRADE_PROGRESS_REPORT,
		EventTime: timestamp.Timestamp(),
		Paras:     progress,
	}
	data := iot.EventData{
	...
	if token := device.Client.Publish(mqtt2.FormatTopic(iot.DeviceToPlatformTopic, device.Id), device.qos, false, convert.ToString(data)); token.Wait() && token.Error() != nil {
