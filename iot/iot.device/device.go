...
func (device *iotDevice) SendMessage(message iot.Message) bool {
	...
	if token := device.base.Client.Publish(iot.FormatTopic(topics.MessageUpTopic, device.base.Id), device.base.qos, false, messageData); token.Wait() && token.Error() != nil {
...
func (device *iotDevice) ReportDeviceInfo(version *iot.VersionInfo) {
	...
	device.base.Client.Publish(iot.FormatTopic(topics.DeviceToPlatformTopic, device.base.Id), device.base.qos, false, iot.Interface2JsonString(request))
}
